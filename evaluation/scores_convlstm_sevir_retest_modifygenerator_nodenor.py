...
class Model_eval_RNN(object):
    ...
    def eval(self, dataloader, model, epoch):
        ...
        with torch.no_grad():
            ...
            info = 'Test EPOCH INFO: epoch:{} \nMSE:{:.5f}  MAE:{:.5f}  SSIM:{:.5f}  PSNR:{:.5f}  MSE_MAE:{:.5f}\nCSI_16:{:.5f}  CSI_74:{:.5f}  CSI_133:{:.5f}  CSI_160:{:.5f}  CSI_181:{:.5f}  CSI_219:{:.5f}\nHSS_16:{:.5f}  HSS_74:{:.5f}  HSS_133:{:.5f}  HSS_160:{:.5f}  HSS_181:{:.5f}  HSS_219:{:.5f}\n'. \
            ...
            with open(os.path.join(self.args.record_dir, 'record.txt'), 'a') as f:
                f.write(info)
                f.write(f"Avg_CSI: {avgcsi:.5f}\tAvg_HSS: {avghss:.5f}\n\n")
