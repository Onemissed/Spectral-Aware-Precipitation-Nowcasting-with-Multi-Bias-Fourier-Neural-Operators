...
class BasicConv2d(nn.Module):
    def __init__(self,
        ...
        # 注意：此处的nn.PixelShuffle方法用于图像上采样
        if upsampling is True:
            ...
            #     stride=stride, padding=5 // 2, dilation=dilation)
            self.conv2 = nn.Conv2d(
            ...
            # self.norm4 = nn.GroupNorm(2, out_channels)
            # self.fuse = nn.Conv2d(3 * out_channels, out_channels, 1)
        else:
            self.conv = nn.Conv2d(
...
class AFNO_model(nn.Module):
    def __init__(self, args, in_shape, hid_S=16, hid_T=256, N_S=4, N_T=4,
        ...
        act_inplace = False
        # if args.output_length >= args.input_length:
        #     k = math.ceil(args.output_length / args.input_length)
        # else:
        #     k = 1
        # self.k = k
...
class Decoder(nn.Module):
    ...
    def forward(self, hid, enc1=None):
        ...
        # 注意：这里只选取encoder浅层特征的最后一帧复制T'次，作为残差
...
class AFNO_SubBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.input_dim == self.hidden_dim:
            ...
            # feature_spatial = x_spatial + self.inception(F.silu(self.batch_norm(x_spatial)))
...
# 重点：ANFO模块中使用的滤波器（用于替换Transformer中的自注意力机制
class AFNO2D(nn.Module):
    ...
    def forward(self, x, spatial_size=None):
        ...
        """注意：这里使用了激活函数relu，导致实部和虚部中的负数变为0"""
...
class AFNOBlock(nn.Module):
    def __init__(self, T, C_in, C_hid, spatio_kernel, embed_dim, mlp_ratio, drop_rate, drop_path, norm_layer=nn.LayerNorm, use_fno=False, use_blocks=False, act_inplace=True):
        ...
        # 重点：GASubBlock系列块的前向传播过程(与VAN完全相同)
    def forward(self, x):
        B, C, H, W = x.shape
